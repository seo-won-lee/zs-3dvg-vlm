...
# Reconstructed queries from parsed target/anchor, with or without original captions
# Whether to include original captions is controlled by 'include_orig_query' flag
# --------------------------------------------------------
...
def process_room(
    ...
    mask3d_bboxes = load_bboxes(room, pred_bbox_dir, "pred") # mask3d 예측 결과 아님! 
    object_names = [obj["target"] for obj in mask3d_bboxes.values()]

    # Generate objects information (pred)
    ...
    for i, d in enumerate(tqdm(queries)):
        ...
        try:
            ...
            target_des = d["parsed_query"]["TargetDescription"]
            relation_raw = d["parsed_query"]["Relation"]
            anchor_raw = d["parsed_query"]["Anchor"]
        except:
            target_name = ""
            anchor_name = ""
            target_des = ""
        ...
        new_query = reconstruct_query(
            ...
            include_orig_query=True  # or None
        )
        ...
        if len(targets) == 0:   # target 없는 경우 -> target을 모든 bbox 결과물로 설정 => multiple target 될 수!
            targets = list(mask3d_bboxes.values())

        if len(anchors) == 0: # anchor가 없는 경우 -> target이 anchor 역할
            anchors = targets
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        # required=True,
        # default="./outputs/qwen2-vl-7b/nr3d/parsed_w_caption",  # uses original caption + reconstructed query
        default="./outputs/qwen2-vl-7b/nr3d/parsed_wo_caption",  # uses reconstructed query only (no original caption)
    ...
    parser.add_argument(
        "--gt_bbox_dir",
        # required=True,
        default="./data/nr3d/object_lookup_table/gt",
        help="Ground truth bounding box directory",
    )
    parser.add_argument(
        "--pred_bbox_dir",
        # required=True,
        default="./data/nr3d/object_lookup_table/pred",
