...
# View ablation with CLIP-based ranking of multi-view images
# Top-K views are selected for inference
# Camera sampling (azimuth_step) is defined in multiview_projection
# Number of input views is controlled via get_topk_views(scores, k)
# --------------------------------------------------------
...
from inference.multiview_projection import render_point_cloud_with_pytorch3d_with_objects_multiview
...
def get_topk_views(scores_for_query, k=3):
    """
    scores_for_query: dict(image_path -> score)
    k: number of top views to return
    """
    # score - descending order
    sorted_items = sorted(
        scores_for_query.items(),
        key=lambda x: x[1],
        reverse=True
    )
    ...
    image_path_list = [path for path, score in topk]
    return image_path_list

def reconstruct_query(target, anchor_queries, include_orig_query=None):
    parts = []
    if include_orig_query:
        parts.append(include_orig_query)
    parts.append(f"Find {target}.")
    for aq in anchor_queries:
        parts.append(f"{target} is {aq}.")
    return " ".join(parts)
...
def process_room(
    ...
    for i, d in enumerate(tqdm(queries)):
        ...
        new_query = reconstruct_query(
            target=target_query,
            anchor_queries=anchor_queries,
            include_orig_query=query  # or None
        )
        ...
        image_path_list, caamera_list = render_point_cloud_with_pytorch3d_with_objects_multiview(
            mask3d_bboxes.values(),
            targets,
            anchors,
            center,
            scan_pc,
            save_dir=f"./projection_img/{dataset}/qwen2-vl-7b/{room}/{i}",
        ...
        for j, image_path in enumerate(image_path_list):
            ...
            if len(target_feature) > 0:
                ...
                target_score = 0.0
            scores_for_query[image_path] = float(target_score)

        best_image_list = get_topk_views(scores_for_query, k=2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        # required=True,
        default="./outputs/qwen2-vl-7b/scanrefer/top3_azim90_target",
